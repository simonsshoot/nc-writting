...
plt.subplots_adjust(wspace=0.2, hspace=0.3, top=0.94,left=0.05, right=0.98)

def plot_bar(ax, cat_labels, val1, val2, title, color1, color2, label1, label2):
    x = np.arange(len(cat_labels))
    width = 0.4
    
    rects1 = ax.bar(x - width/2, val1, width, label=label1, color=color1, edgecolor='white')
    rects2 = ax.bar(x + width/2, val2, width, label=label2, color=color2, edgecolor='white')
    
    ax.set_title(title, fontsize=16, fontweight='bold', pad=15)
    ax.set_xticks(x)
    ax.set_xticklabels(cat_labels, rotation=30)
    ax.set_ylim(0, 110)
...
for i, model in enumerate(models):
    ...
    plot_bar(axes[0, i], categories, need_vals, noneed_vals, 
             f'{model}: Need vs No Need', '#2E5A88', '#A8DADC', 'Need', 'No Need')
    
    # 第二行：New vs Reused
    new_vals = data_new[model]
    reused_vals = [100 - v for v in new_vals] # 注：图片中部分数据不加和为100，此处以New数据为准计算对比
    plot_bar(axes[1, i], categories, new_vals, reused_vals, 
             f'{model}: New vs Reused', '#D85C27', '#FFB385', 'New', 'Reused')
