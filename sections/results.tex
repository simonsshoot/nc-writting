\section{Results}
\label{sec:results}
In this section, we present a comprehensive evaluation of the EVOLVE framework. We first report its overall performance across three benchmarks covering diverse domains, backbone LLMs, and task categories (\S\ref{subsec:benchmark_performance}). We then conduct ablation studies (\S\ref{subsec:ablation_study}) to verify the necessity of each core component. In the analysis section (\S\ref{subsec:analysis}), we examine EVOLVE's adaptability to embodied intelligence scenarios, its guardrail tool self-evolution mechanism, and the role of the auditing mechanism in mitigating vulnerabilities during evolution. Finally, we deploy EVOLVE in two real-world system (\S\ref{subsec:real_world}) to demonstrate its practical applicability. Cases of generated guardrail tools are in Appendix~\ref{appendix:toolcase} and cases of tool evolution are in Appendix~\ref{appendix:toolevolutioncase}.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{code/ablation_study_temp.pdf}
...
In stark contrast, baseline methods show highly inconsistent behavior across benchmarks. ShieldAgent achieves an F1 score of 0.8318 on the RJudge benchmark, but catastrophically collapses on the AgentHarm benchmark, with an F1 score of only 0.1791, a drop of over 65\%. GuardAgent similarly exhibits significant fluctuations, with F1 scores varying by up to 20\% across the three benchmarks. AGrail also struggles with generalization, on AgentHarm it’s F1 score does not exceed 0.5. Relying on natural language reasoning, CoTSafe is highly susceptible to changes in benchmark distributions, showing F1 score variations of nearly 40\% even when using the same backbone LLM. The root cause of these failures is their reliance on static, pre-defined safety rules or natural language prompts, which cannot adapt when the risk distribution shifts across benchmarks. EVOLVE avoids this pitfall through its dynamic tool generation pipeline: AnalysisAgent identifies emerging risk patterns on the fly, and the generated guardrail tools encode scenario-specific detection logic as executable code, allowing the defense to co-evolve with the threat landscape rather than degrade under distributional shift.
